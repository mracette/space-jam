...
const PRODUCTION_BUILD_COMMAND = "yarn build:prod";
...
function runHook(args) {
  ...
  LOG_MESSAGE += `| Compressed Build | ${kb2} kb | ${bytes2} | ${
    bytes2 < bytes1 ? "-" : "+"
  }${Math.abs(Math.round((100 * (bytes1 - bytes2)) / bytes1))}% |\n`;
  ...
  LOG_MESSAGE += `| Compressed Build (Adv Zip) | ${kb3} kb | ${bytes3} | ${
    bytes3 < bytes2 ? "-" : "+"
  }${Math.abs(Math.round((100 * (bytes2 - bytes3)) / bytes2))}% |\n`;
  ...
  // stage log changes
  execSync("git add " + COMMIT_LOG_PATH);
}
